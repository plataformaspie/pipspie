...
class DashboardController extends Controller
{
  public function __construct()
  {
      ...
      $sql = \DB::select("SELECT m.* FROM menus m INNER JOIN roles_menu rm ON m.id = rm.id_menu WHERE rm.id_rol = ".$rol." AND id_modulo = 3  AND activo = true  ORDER BY m.orden ASC");
      $this->menus = array();
      foreach ($sql as $mn) {
